...
export const createFixture = async (fileName: string) => {
    ...
    const extractRef = flow(
        O.fromPredicate(R.startsWith('__ref')),
        O.map(R.split('.')),
    ...
    for await (const [key, val] of R.toPairs(fixture)) {
        if (key === 'generate') {
            for (const generateKey in val) {
                if (!(val[generateKey] in generateTypeMap)) throw new Error("not a generatable type")
                generateMap.set(generateKey, generateTypeMap[val[generateKey] as GenerateType]())
            }
        } else {
